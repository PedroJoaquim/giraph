...
import org.apache.giraph.checkpointing.HDFSOptimizedCheckpointHandler;
...
public class BspServiceMaster<I extends WritableComparable,
    ...
    /** Master Checkpoint Handler */
    private final MasterCheckpointHandler<I, V, E> masterCheckpointHandler;
    ...
    public BspServiceMaster(
        ...
        this.masterCheckpointHandler = getCheckpointHandler().
                createMasterCheckpointHandler(this, this, getCheckpointPathManager());
    ...
    /**
     *
     * @return the emr cluster ID of the machine
     */
    ...
    public int createCheckpointInputSplits() {
        ...
        if(getRestartedSuperstep() != BspService.UNSET_SUPERSTEP){
            ...
            CheckpointInputFormat<I, E> verticesCheckpointInputFormat =
                    getConfiguration().createWrappedCheckpointInputFormat(
                            getCheckpointPathManager().getVerticesCheckpointFilesDirPath(getRestartedSuperstep(), true));

            CheckpointInputFormat<I, E> messagesCheckpointInputFormat =
                    getConfiguration().createWrappedCheckpointInputFormat(
                            getCheckpointPathManager().getMessagesCheckpointFilesDirPath(getRestartedSuperstep(), true));


            splits += createInputSplits(verticesCheckpointInputFormat, InputType.CHECKPOINT_VERTICES);

            splits += createInputSplits(messagesCheckpointInputFormat, InputType.CHECKPOINT_MESSAGES);
        }
    ...
    private Collection<PartitionOwner> prepareCheckpointRestart(long superstep)
        ...
        return this.masterCheckpointHandler.prepareCheckpointRestart(superstep);
    }
    ...
    private void finalizeCheckpoint(long superstep,
        ...
        this.masterCheckpointHandler.finalizeCheckpoint(superstep, chosenWorkerInfoList);
    ...
    private void doMETISPartitioning() {
        ...
        if(getConfiguration().isUndirectedGraph()){
            ...
            writeMetisProblemUndirected();
            long end2 = System.currentTimeMillis();
            LOG.info("debug-metis: time writing metis problem " + (end2 - end)/1000.0d + " secs");
        }
        else {
            Int2LongOpenHashMap[] partitionAdjacencyInfo = readMetisPartitionsInfo();
            long end2 = System.currentTimeMillis();
    ...
    //adjacencies are simmetrical
    private void writeMetisProblemUndirected() {
        ...
        for (int i = 1; i <= getWorkerInfoList().size(); i++) {
            ...
            try {
                ...
                edgeInfo[0] += fileInputStream.readLong();
                edgeInfo[1] += fileInputStream.readLong();
    ...
    private int[] runMetisPartitioning() {
        ...
        try {
            ...
            Scanner sc = new Scanner(new File(metisOutputFile));
            int partitionId = 0;
    ...
    private void writeMetisProblem(Int2LongOpenHashMap[] partitionsEdgesInfo) {
        ...
        for (int i = 0; i < partitionsEdgesInfo.length; i++) {
            ...
            Int2LongOpenHashMap partitionMap = partitionsEdgesInfo[i];

            ObjectIterator<Int2LongMap.Entry> entryObjectIterator = partitionMap.int2LongEntrySet().iterator();

            while (entryObjectIterator.hasNext()) {

                Int2LongMap.Entry next = entryObjectIterator.next();

                int targetPartitionId = next.getIntKey();
                long numEdges = next.getValue();

                boolean hasReverseEdge = partitionsEdgesInfo[targetPartitionId].containsKey(i);

                //update mine and the other
                if(!hasReverseEdge || targetPartitionId >= i){
                    long reverseNumEdges = partitionsEdgesInfo[targetPartitionId].get(i);
                    next.setValue(reverseNumEdges + numEdges);
                    partitionsEdgesInfo[targetPartitionId].put(i, reverseNumEdges + numEdges);
                    metisNumEdges += 2;
                }
            }
        }

        metisNumEdges = metisNumEdges/2;

        long end = System.currentTimeMillis();

        LOG.info("debug-metis: master time to fix partition info = " + (end - start)/1000.0d + " secs");

        /*long start1 = System.currentTimeMillis();
        writeMETISFileSync(numVertices, metisNumEdges, targetMetisInputFile, partitionsEdgesInfo);
        long end1 = System.currentTimeMillis();

        LOG.info("debug-metis: time to write file SYNC = " + (end1 - start1)/1000.0d + " secs");*/

        long start2 = System.currentTimeMillis();
        writeMETISFilePar(numVertices, metisNumEdges, targetMetisInputFile, partitionsEdgesInfo);
        long end2 = System.currentTimeMillis();

        LOG.info("debug-metis: time to write file PARALLEL = " + (end2 - start2)/1000.0d + " secs");


        long nulFullEdges = ((numVertices*numVertices)-numVertices)/2;

        LOG.info("debug-metis: fullConnectedGraph = " + nulFullEdges +
                " numEdges = " + metisNumEdges + " (" + ((metisNumEdges*100)/nulFullEdges) + "%)");
    }

    private void writeMETISFilePar(int numVertices,
                                   long metisNumEdges,
                                   String targetMetisInputFile,
                                   final Int2LongOpenHashMap[] partitionsEdgesInfo) {


        int numThreads = Math.min(
                GiraphConstants.NUM_CHECKPOINT_IO_THREADS.get(getConfiguration()),
                partitionsEdgesInfo.length);

        int numWorkItems = Math.min(numThreads * 10, partitionsEdgesInfo.length);

        final ConcurrentLinkedQueue<int[]> queue =
                generateMETISThreadsWork(partitionsEdgesInfo, numWorkItems);
                        ...
                        while (!queue.isEmpty()) {
                            ...
                            for (int i = startIdx; i < endIdx; i++) {
                                ...
                                while (entryObjectIterator.hasNext()){
                                    ...
                                    long weight = next.getLongValue();

                                    workSb.append(key+1).append(" ").append(weight).append(" ");
                                }

                                workSb.setLength(workSb.length() - 1);

                                if(i != partitionsEdgesInfo.length - 1){
                                    workSb.append("\n");
                                }
                            }
        ...
        try {
            ...
            writer.println(numVertices + " " + metisNumEdges + " 001");

            for (int i = 0; i < sbArray.length; i++) {
                writer.print(sbArray[i].toString());
            }
    ...
    private ConcurrentLinkedQueue<int[]> generateMETISThreadsWork(Int2LongOpenHashMap[] partitionsEdgesInfo, int numWorkItems) {

        ConcurrentLinkedQueue<int[]> work = new ConcurrentLinkedQueue<>();

        int workPerWorkItem = partitionsEdgesInfo.length / numWorkItems;

        for (int i = 0; i < numWorkItems; i++) {

            if(i == numWorkItems - 1){
                work.add(new int[]{i, (i * workPerWorkItem), partitionsEdgesInfo.length});
            }
            else {
    ...
    private void writeMETISFileSync(int numVertices,
                                    long metisNumEdges,
                                    String targetMetisInputFile,
                                    Int2LongOpenHashMap[] partitionsEdgesInfo) {
    ...
    private Int2LongOpenHashMap[] readMetisPartitionsInfo()  {
        ...
        LOG.info("debug-metis: random micro partitioning edge cut = " +
                (getConfiguration().isUndirectedGraph() ? totalRandomEdgeCut/2 : totalRandomEdgeCut));
    ...
    private void coordinateInputSplits() {
        ...
        LOG.info("debug-metis: time to load input splits = " + (end - start)/1000.0d + " secs");
